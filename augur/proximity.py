...
def load_context(fname: str, skip_strains: set[str], seq_len: int) \
    ...
    context_matrix = context_matrix[:n]  # trim to actual size
...
def _make_progress_callback(total: int) -> Callable[[], None]:
...
def run(args: argparse.Namespace) -> None:
    ...
    _process_queries_kwargs = dict(
        ...
        on_query_done=_make_progress_callback(len(query_items)),
