...
proximity_argument_descriptions: dict[str, str] = {
    ...
    'sequences': "FASTA file with aligned contextual sequences",
    'query': "FASTA file with aligned query sequences to find neighbors for",
    'output_strains': "output file with one neighbor strain name per line",
    'output_matches': "optional TSV file with columns: query strain, context strain, distance",
    'output_sequences': "All proximal strains which match the queries",
    'k': "number of nearest neighbors to find per query strain",
    ...
    'nthreads': "Number of threads to use for parallel query processing. Use 'auto' to use all available cores.",
}
"""
...
def register_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument('--sequences', required=True, help=proximity_argument_descriptions['sequences'])
    parser.add_argument('--query', required=True, help=proximity_argument_descriptions['query'])
    ...
    for record in read_sequences(fname):
        ...
        if not seq_len:
            ...
            raise AugurError(dedent(f"""\
                When reading query samples, {record.name!r} is {len(q):,}nt, which differs
                from previous query sequences ({seq_len:,}nt). Query sequences must be aligned
    ...
    for record in read_sequences(fname):
        ...
        if len(arr) != seq_len:
            raise AugurError(dedent(f"""\
                ...
                from previous query sequences ({seq_len:,}nt). All sequences must be aligned and
...
def _process_queries(
    ...
    *ignore_missing_data* controls N handling ("none", "all", or "flanking").
    ...
    for query_strain, query_seq in query_items:
        ...
        compute_batch = distance_fn(ignore_missing_data, query_seq, context_matrix, query_valid_range, context_valid_mask)
...
def run(args: argparse.Namespace) -> None:
    ...
    if args.output_sequences:
        subset_fasta(args.sequences, args.output_sequences, args.output_strains, args.nthreads)
