...
def validate_json(jsonToValidate, schema, filename):
    ...
    def custom_message(error):
        ...
        if error.absolute_path and validator in {"oneOf", "anyOf"}:
            return f"{short_value} is invalid, see below."

        if validator == "required":
            missing_property = str(error.args[0]).split("'")[1]
            return f"Missing required field '{missing_property}'"
        ...
        if validator == "pattern":
            ...
            if "[0-9X]{4}-[0-9X]{2}-[0-9X]{2}" in pattern:
