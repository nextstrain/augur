...
def register_arguments(parser):
    ...
    group_size_options.add_argument('--group-by-weights', type=str, metavar="FILE", help="""
        ...
        (2) There must be a numeric 'weight' column (weights can take on any non-negative values).
        (3) Other columns must be a subset of columns used in `--group-by`, with combinations of values covering all combinations present in the metadata.
        (4) This option only applies when `--group-by` and `--subsample-max-sequences` are provided.
        (5) This option cannot be used with `--probabilistic-sampling`.

        Any `--group-by` columns not present in the file will be given equal weighting across all values.
    """)
