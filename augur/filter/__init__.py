...
def register_arguments(parser):
    ...
    input_group.add_argument('--sequences', '-s',
        help="sequences in FASTA or VCF format")
    input_group.add_argument('--sequence-index',
        help="""sequence composition report generated by augur index. If not
             provided, an index will be created on the fly.""")
    input_group.add_argument('--metadata-chunk-size', type=int, default=100000,
        help="""maximum number of metadata records to read into memory at a
             ...
             more memory used.""")
    input_group.add_argument('--metadata-id-columns', default=DEFAULT_ID_COLUMNS, nargs="+",
        help="""names of possible metadata columns containing identifier
             information, ordered by priority. Only one ID column will be
             inferred.""")
    input_group.add_argument('--metadata-delimiters', default=DEFAULT_DELIMITERS, nargs="+",
    ...
    metadata_filter_group.add_argument('--query',
        help="""Filter samples by attribute. Uses Pandas Dataframe querying, see
    ...
    metadata_filter_group.add_argument('--min-date', type=numeric_date_type,
        help=f"""minimal cutoff for date, the cutoff date is inclusive; may be
              specified as: {SUPPORTED_DATE_HELP_TEXT}""")
    metadata_filter_group.add_argument('--max-date', type=numeric_date_type,
    ...
    metadata_filter_group.add_argument('--exclude-ambiguous-dates-by', choices=['any', 'day', 'month', 'year'],
        help="""Exclude ambiguous dates by day (e.g., 2020-09-XX), month (e.g.,
    ...
    metadata_filter_group.add_argument('--exclude', type=str, nargs="+",
    ...
    metadata_filter_group.add_argument('--include', type=str, nargs="+",
    ...
    sequence_filter_group.add_argument('--min-length', type=int,
        help="""minimal length of the sequences, only counting standard
             nucleotide characters A, C, G, or T (case-insensitive)""")
    sequence_filter_group.add_argument('--max-length', type=int,
        help="""maximum length of the sequences, only counting standard
             nucleotide characters A, C, G, or T (case-insensitive)""")
    ...
    subsample_group.add_argument('--group-by', nargs='+',
    ...
    subsample_limits_group.add_argument('--sequences-per-group', type=int,
        help="""subsample to no more than this number of sequences per
             category""")
    subsample_limits_group.add_argument('--subsample-max-sequences', type=int,
        help="""subsample to no more than this number of sequences; can be used
             without the group_by argument""")
    ...
    subsample_group.add_argument('--priority', type=str,
    ...
    subsample_group.add_argument('--subsample-seed', type=int,
    ...
    output_group.add_argument('--output', '--output-sequences', '-o',
        help="filtered sequences in FASTA format")
    output_group.add_argument('--output-metadata',
        help="metadata for strains that passed filters")
    output_group.add_argument('--output-strains',
        help="list of strains that passed filters (no header)")
    output_group.add_argument('--output-log',
        help="""tab-delimited file with one row for each filtered strain and the
             ...
             are reported in JSON format in a `kwargs` column.""")
