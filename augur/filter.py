...
def run(args):
    ...
    is_vcf = False
    is_compressed = False
    if args.sequences and any([args.sequences[0].lower().endswith(x) for x in ['.vcf', '.vcf.gz']]):
        is_vcf = True
        if args.sequences[0].lower().endswith('.gz'):
            is_compressed = True
    ...
    if is_vcf:
        vcf_sequences, _ = read_vcf(args.sequences[0])
    ...
    if is_vcf:
        if args.output:
            ...
            write_vcf(args.sequences[0], args.output, dropped_samps)
    elif args.sequences:
