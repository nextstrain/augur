...
def register_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    input_group.add_argument('--metadata', metavar="FILE", type=resolvable_filepath, help="sequence metadata" + SKIP_AUTO_DEFAULT_IN_HELP)
    input_group.add_argument('--sequences', metavar="FILE", type=resolvable_filepath, help="sequences in FASTA or VCF format. For large inputs, consider using --sequence-index in addition to this option." + SKIP_AUTO_DEFAULT_IN_HELP)
    input_group.add_argument('--sequence-index', metavar="FILE", type=resolvable_filepath, help="sequence composition report generated by augur index. If not provided, an index will be created on the fly. This should be generated from the same file as --sequences." + SKIP_AUTO_DEFAULT_IN_HELP)
    ...
    config_group.add_argument("--config", metavar="FILE", required=True, type=resolvable_filepath, help="augur subsample config file. The expected config options must be defined at the top level, or within a specific section using --config-section." + SKIP_AUTO_DEFAULT_IN_HELP)
...
class ProximalSample(Sample):
    ...
    def connect_dependencies(self, samples_by_name: Dict[str, Sample]) -> None:
        ...
        # A proximal sample may specify a specific contextual sample (as opposed to the global sequences)
        # We need to retrieve the contextual sample and ensure it writes out a sequences file that
        # this proximal sample can consume.
        context_sample = None
        if 'context_sample' in self.depends_on:
            ...
            if type(context_sample) is not FilterSample:
                ...
                proximity sample!
                """))
            if 'sequences' not in context_sample.outputs:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while running or ready:
            ...
            if not running and ready:
                sample_name = ready[0]
                sample = by_name[sample_name]
                raise AugurError(f"Sample {sample.name!r} cannot be scheduled: needs {sample.nthreads} threads but only {threads_available} are free.")
