...
def run(args: argparse.Namespace) -> None:
    ...
    if len(samples) == 1:
        ...
        del samples[0].filter_args["--output-strains"]
        ...
        try:
            # Run intermediate augur filter calls.
...
class Sample:
    ...
    def run(self) -> None:
        ...
        # Output console messages only after the command has completed.
...
def _add_to_args(args: FilterArgs, filter_flag: AugurFilterOption, value: Any) -> None:
    """Add a filter flag and its value to the arguments dictionary."""
...
def _args_dict_to_list(args: FilterArgs) -> List[str]:
    ...
    for filter_flag, value in args.items():
        # No value means a boolean flag.
        if value is None:
            args_list.append(filter_flag)

        # Lists and tuples are unpacked and given as strings.
        elif isinstance(value, (list, tuple)):
            args_list.extend([filter_flag, *(str(v) for v in value)])
...
def _run_final_filter(filter_args: FilterArgs) -> None:
    ...
    This final call can go through the current Python process to take advantage
