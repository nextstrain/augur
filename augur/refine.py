...
def register_parser(parent_subparsers):
    ...
    parser.add_argument('--aa', action="store_true", help="the input alignment is amino acid (protein). Uses the JTT92 substitution model.")
...
def run(args):
    ...
    gtr = 'JTT92' if alphabet == 'aa' else 'JC69'
    ...
    if not args.alignment:
        ...
        dummy_seq = 'ACDE' if alphabet == 'aa' else 'ACGT'
        seqs = []
        ...
        if alphabet == 'aa':
            raise AugurError("VCF alignments are not supported with --aa. Please provide a FASTA alignment.")
    ...
    if args.divergence_units=='mutations-per-site': #default
        ...
        seq_map = profile_maps[alphabet]
        ambiguous_char = 'X' if alphabet == 'aa' else 'N'
