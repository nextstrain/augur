...
def merge_sequences(args):
        ...
        for s in args.sequences:
            ...
            with NamedTemporaryFile("w+", encoding = "utf-8") as dup_num_file:
                ...
                try:
                    ...
                    raise AugurError(f"Validation failed for {s!r}. See error above.") from error

                dup_num_file.seek(0)

                if duplicates := list(parse_seqkit_dup_num_file(dup_num_file.read().strip())):
    ...
    try:
        ...
        if os.path.exists(args.output_sequences):
...
def parse_seqkit_dup_num_file(contents: str):
    ...
    >>> contents = "3\\tid,, id,, id,\\n2\\tid1, id1\\n"
    >>> list(parse_seqkit_dup_num_file(contents))
    ['id,', 'id1']
    ...
    >>> list(parse_seqkit_dup_num_file(""))
    []
    """
    if contents == "":
        return
