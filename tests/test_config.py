...
def test_get_referenced_files_search_paths(tmp_path):
    """
    Test get_referenced_files with custom search_paths.
    """
    sub_dir = tmp_path / "custom_dir"
    sub_dir.mkdir()
    alignment = sub_dir / "alignment.fasta"
    alignment.touch()
    ...
    assert get_referenced_files(config_file, search_paths=[str(sub_dir)]) == {str(alignment.resolve())}
