...
class TestHammingWithMisingData:
    ...
    def test_count_all_ns(self, query_fasta, sequences_fasta, output_file, argparser):
        max_dist = 10
        args = argparser(f"--method hamming --ignore-missing-data none --query {query_fasta(self.QUERY)} --sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 100 --max-distance {max_dist}")
    ...
    def test_ignore_all_ns(self, query_fasta, sequences_fasta, output_file, argparser):
        max_dist = 3
        args = argparser(f"--method hamming --ignore-missing-data all --query {query_fasta(self.QUERY)} --sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 100 --max-distance {max_dist}")
    ...
    def test_ignore_flanking_ns(self, query_fasta, sequences_fasta, output_file, argparser):
        max_dist = 3
        args = argparser(f"--method hamming --ignore-missing-data flanking --query {query_fasta(self.QUERY)} --sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 100 --max-distance {max_dist}")
