...
class TestHammingSimple:
    ...
    FOCAL = {"focal": BASE_GENOME}
    SEQS = {}
    for i in range(1,11): # no dict comprehension - BASE_GENOME out of scope
        SEQS[f"seq_{i}"] = str(Genome(BASE_GENOME).mutate(i))

    def test_run_finds_correct_neighbours(self, focal_fasta, sequences_fasta, output_file, argparser):
        """With k=5 and a high max-distance, the 5 closest sequences should be returned."""
        args = argparser(f"--method hamming --focal-sequences {focal_fasta(self.FOCAL)} --context-sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 5 --max-distance 10")
        proximity.run(args)
        result = read_output(output_file)
        assert sorted(result) == ["seq_1", "seq_2", "seq_3", "seq_4", "seq_5"]

    def test_run_k_limits_output(self, focal_fasta, sequences_fasta, output_file, argparser):
        """k=3 should return only the 3 closest sequences."""
        args = argparser(f"--method hamming --focal-sequences {focal_fasta(self.FOCAL)} --context-sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 3 --max-distance 10")
        proximity.run(args)
        result = read_output(output_file)
        assert sorted(result) == ["seq_1", "seq_2", "seq_3"]

    def test_run_max_distance_arg(self, focal_fasta, sequences_fasta, output_file, argparser):
        """With max-distance of 3 we should only get the first three sequences"""
        args = argparser(f"--method hamming --focal-sequences {focal_fasta(self.FOCAL)} --context-sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 10 --max-distance 3")
        proximity.run(args)
        result = read_output(output_file)
        assert sorted(result) == ["seq_1", "seq_2", "seq_3"]

    def test_run_excludes_focal_from_context(self, focal_fasta, sequences_fasta, output_file, argparser):
        """If the focal strain also appears in the context FASTA, it should be excluded."""
        # Create a context FASTA that includes the focal sequence
        records = {**self.FOCAL}
        records.update({**self.SEQS})
        args = argparser(f"--method hamming --focal-sequences {focal_fasta(self.FOCAL)} --context-sequences {sequences_fasta(records)} --output-strains {output_file} --k 5 --max-distance 10")
        proximity.run(args)
        result = read_output(output_file)
...
class TestHammingWithMisingData:
    ...
    def test_count_all_ns(self, focal_fasta, sequences_fasta, output_file, argparser):
        max_dist = 10
        args = argparser(f"--method hamming --ignore-missing-data none --focal-sequences {focal_fasta(self.FOCAL)} --context-sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 100 --max-distance {max_dist}")
        expected = [k for k,v in self.SEQS_INFO.items() if len(v.mutated_positions)+v.num_ns <= max_dist]
        proximity.run(args)
        result = read_output(output_file)
        assert sorted(result) == sorted(expected)

    def test_ignore_all_ns(self, focal_fasta, sequences_fasta, output_file, argparser):
        max_dist = 3
        args = argparser(f"--method hamming --ignore-missing-data all --focal-sequences {focal_fasta(self.FOCAL)} --context-sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 100 --max-distance {max_dist}")
        expected = [k for k,v in self.SEQS_INFO.items() if len(v.mutated_positions) <= max_dist]
        proximity.run(args)
        result = read_output(output_file)
        assert sorted(result) == sorted(expected)

    def test_ignore_flanking_ns(self, focal_fasta, sequences_fasta, output_file, argparser):
        max_dist = 3
        args = argparser(f"--method hamming --ignore-missing-data flanking --focal-sequences {focal_fasta(self.FOCAL)} --context-sequences {sequences_fasta(self.SEQS)} --output-strains {output_file} --k 100 --max-distance {max_dist}")
