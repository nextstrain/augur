...
class TestFilter:
    ...
    def test_filter_run_min_date_ambiguous(self, tmpdir, fasta_fn, argparser):
        """Test that filter --min-date accepts ambiguous dates"""
        out_fn = str(tmpdir / "out.fasta")
        min_date = "2019"
        meta_fn = write_metadata(tmpdir, (("strain","date"),
                                          ("SEQ_1","2018-XX-XX"),
    ...
    def test_filter_run_min_date_ambiguous_2(self, tmpdir, fasta_fn, argparser):
        """Test that filter --min-date accepts ambiguous dates"""
        out_fn = str(tmpdir / "out.fasta")
        min_date = "2019-04"
        meta_fn = write_metadata(tmpdir, (("strain","date"),
    ...
    def test_filter_run_max_date_ambiguous_2(self, tmpdir, fasta_fn, argparser):
        """Test that filter --max-date accepts ambiguous dates"""
        out_fn = str(tmpdir / "out.fasta")
        max_date = "2019-04"
        meta_fn = write_metadata(tmpdir, (("strain","date"),
