...
Check the output from the above command against its expected contents
  $ python3 "$TESTDIR/../../../../scripts/diff_jsons.py"  "$TESTDIR/../data/dataset2.json" dataset5.json \
  >   --exclude-paths "root['meta']['updated']"
  {}
...
mutations (node_data→nodes) and a historical node_data→nodes→<name>→clade_annotation branch label.
  ...
  >   --maintainers "Nextstrain Team" \
  >   --output dataset-with-branch-labels.json > /dev/null

  $ python3 "$TESTDIR/../../../../scripts/diff_jsons.py"  "$TESTDIR/../data/dataset-with-branch-labels.json" dataset-with-branch-labels.json \
  >   --exclude-paths "root['meta']['updated']"
  {}
