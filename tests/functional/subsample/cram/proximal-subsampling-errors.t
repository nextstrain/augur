...
We don't allow proximity sampling where the query set are themselves a proximal sample
  ...
  ERROR: The proximal sample 'prox_from_prox' declares a focal sample of 'prox'
...
We don't allow proximity sampling where the context set is a proximal sample
  ...
  >   --seed 0
  Validating schema of 'samples1.yaml'...
  ERROR: The proximal sample 'prox_two' declares a context sample of 'prox_one'
  which must itself be a non-proximal sample. I.e. you can't proximity sample directly from another
  proximity sample!
...
Because there are multiple outbreak samples, it's the unaligned focal seqs which
trigger the error.
  ...
  [prox] ERROR: When reading focal samples, 'DOM/2016/BB_0183' is 10,621nt, which differs
  [prox] from previous focal sequences (10,035nt). Focal sequences must be aligned
  [prox] for proximity calculations.
  ERROR: Sample 'prox' failed, see error above.
  [2]

The same as above, but using a single outbreak sequence so the focal is "aligned"
(as it's n=1) and thus the error is thrown when reading the --sequences
  ...
  \[prox\] Read 1 focal sequences from .* (re)
  [prox] ERROR: When reading sequences, 'PAN/CDC_259359_V1_V3/2015' has length 10,771nt, which differs
  [prox] from previous focal sequences (10,675nt). All sequences must be aligned and
  [prox] the same length for proximity calculations.
  ERROR: Sample 'prox' failed, see error above.
  [2]


Test that a sampling scheme with a proximal sample requires --sequences in the first place
  ...
  >   --seed 0
  Validating schema of 'samples3.yaml'...
  ERROR: Augur subsample with a proximal sample requires (aligned) sequences to be provided.
  [2]
