...
The :doc:`augur subsample <./subsample>` command is purpose-built to do this in a single step, which avoids having to code this logic into Snakemake.
...
``all``
   Positions where *either* the focal or context sequence has an N are ignored
