...
.. glossary::
   ...
   filter sample
      ...
      Configured by specifying filtering parameters (date ranges, queries etc) and (optionally) another sample to target as the input.

   proximal sample
      
      A specific type of sample where we compare a (small) query sample against a (large) target sample akd find the closest genetic matches.
...
.. schema-options-table:: augur/data/schema-subsample-config.json sampleProximityProperties
...
- As samples may depend on other samples (via the ``query_sample`` and ``target_sample``
