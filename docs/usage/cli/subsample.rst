...
.. glossary::
   ...
   sample
      ...
      1. **Process**: Selecting a subset of sequences from a dataset according to
         specific parameters for filtering and subsampling (e.g. minimum/maximum
         date, minimum/maximum sequence length, sample size).
...
This describes the YAML configuration file for ``augur subsample --config``.

.. tip::

    Use ``--config-root`` to read from a configuration file that puts these
    options under another top-level key.
...
- CLI and YAML config options map closely to augur filter options.
   ...
   The following table shows the mapping between ``augur subsample`` sample configuration options and ``augur filter`` options.
